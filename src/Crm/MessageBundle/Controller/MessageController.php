...
use Crm\AdminBundle\Entity\Encodage;
...
class MessageController extends BaseController
{
    ...
    public function newThreadAction()
    {
        ...
        $recipient = null;
        $destinataire = null;
        $repo = $this->container->get('crm_core.create.username')->repoUtilisateur();
        $form = $this->container->get('fos_message.new_thread_form.factory')->create();
        $formHandler = $this->container->get('fos_message.new_thread_form.handler');
         $userManager = $this->container->get('fos_user.user_manager');

         if(!empty($request->query->get('d'))){

            $recipient = Encodage::decrypt($request->query->get('d'));

            if($userManager->findUserByUsername($recipient) != null){

                $user =  $userManager->findUserByUsername($recipient);
                $d = $repo->findOneByCompte($user);
                if($d != null){
                    $destinataire = $d;
                }
            }
